...
export const getEvents = query({
  ...
  handler: async (ctx) => {
    return await ctx.db
      .query("events")
      .filter((q) => q.lt(q.field("iscancelled"), null))
      .collect();
  },
});
...
export const getEventAvailability = query({
  ...
  handler: async (ctx, { eventId }) => {
    ...
    return {
      ...
      purchasedCount,
      activeOffers,
      remaingTickets: Math.max(0, event.totalTickets - totalReserved),
    };
  },
});
  ...
  handler: async (ctx, { eventId, userId }) => {
    ...
    return {
      ...
      message: available
        ...
        : "Added to waiting list - you'll be notified when a ticket becomes availablle",
    };
  },
});
...
export const purchaseTicket = mutation({
  ...
  handler: async (ctx, { eventId, userId, waitingListId, paymentInfo }) => {
    ...
    if (event.iscancelled) {
      ...
      throw new Error("Event is no longer active - cannot purchase ticket");
    }
