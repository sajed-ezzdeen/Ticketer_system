...
const EventForm = ({ mode, initialData }: EventFormProps) => {
  ...
  async function onSubmit(values: FormData) {
    ...
    startTransition(async () => {
      // startTransition is like a pending state  for async actions like promise is going on and it will not block the UI
      try {
        let imageStorageId = null;

        if (selectedImage) {
          imageStorageId = await handleImageUpload(selectedImage); // upload image and get storage id
        }
        ...
        if (mode === "create") {
          ...
          if (!initialData) {
            throw new Error("Initial event data is required for updates mode");
          }
              ...
              {imagePreview || (!removedCurrentImage && currentImageUrl) ? (
                <div className="relative w-32 aspect-square bg-gray-100 rounded-lg">
                  ...
                  <button
                    ...
                    className="absolute -top-2 -right-2 pb-0.5 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-600 transition-colors"
                  >
                    ×
                  </button>
                </div>
