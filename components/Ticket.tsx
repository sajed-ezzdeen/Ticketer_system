...
const Ticket = ({ ticketId }: { ticketId: Id<"tickets"> }) => {
  ...
  return (
    ...
    >
      ...
      {/* Ticket Footer */}
      <div
        className={`${ticket.event.iscancelled ? "bg-red-50" : "bg-gray-50"} px-6 py-4 flex justify-between items-center`}
      >
        <span className="text-sm text-gray-500">
          Purchase Date: {new Date(ticket.purchasedAt).toLocaleString()}
        </span>
        <span
          className={`text-sm font-medium ${ticket.event.iscancelled ? "text-red-600" : "text-blue-600"}`}
        >
